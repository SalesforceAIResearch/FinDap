...
# directly work on id, instead of the specific data points 
...
class PackedDataset(Dataset):
    def __init__(
        ...
        """This class is used to convert a regular dataset to a packed dataset.
        
        Args:
            dataset (Dataset): Regular dataset that implements __len__ and __getitem__.
            tokenizer (Any): The tokenizer used to tokenize the dataset.
            max_input_length (int): Max sequence length.
            pack_length (int, optional): The maximum length of packed data points. If -1, set to max_input_length.
            max_packed_size (int, optional): Maximum number of data points that can be packed. If -1, no limit.
        """
        super().__init__()
        self.tokenizer = tokenizer
        self.max_input_length = max_input_length

        self.pack_length = pack_length if pack_length != -1 else max_input_length

        # Collect lengths and data point indices
        self.lengths = lengths
        size = len(dataset)
        self.data_indices = range(size)


        max_length = max(self.lengths)
        assert self.pack_length >= max_length, \
            f"pack_length must be >= max(input lengths), found pack_length={self.pack_length}, max_length={max_length}"

        # Create groups based on length
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        for index in group:
            data_point = self._fetch_data_point(index)
            ...
            group_data_points.append(n_data_point)
    ...
    def _fetch_data_point(self, index: int) -> Dict:
        # Fetch the data point from the original dataset
        return self.original_dataset[index]

    def stat(self):
        print(
            f"Number of original data points: {len(self.data_indices)}; packed to: {len(self.groups)} data points"
        )
...
# TODO: please double check this is correct!!!